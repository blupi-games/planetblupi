...
bool CDecor::ListPut (Sint32 rank, Sint32 button, POINT cel, POINT cMem)
{
    ...
    if (button == BUTTON_MANGE &&
        m_blupi[rank].listButton[0] != BUTTON_CULTIVE)
        return true;
...
bool CDecor::BlupiGoal (Sint32 rank, Sint32 button, POINT cel, POINT cMem)
{
    ...
    if (action == WM_ACTION_PONTE)
    {
        ...
        m_blupi[rank].nLoop = static_cast<Sint16> (abs ((test.x - cel.x) +
                              (test.y - cel.y)) / 2);
...
bool CDecor::IsBlupiHereEx (POINT cel1, POINT cel2, Sint32 exRank,
                            bool bSkipInMove)
{
    Sint32          rank;
