...
1. **Main Source Code Repository & Development Bundle:**
    * The primary C++ source code for the game is located in the `blupi-games/planetblupi` repository.
    * To build the game, use the `planetblupi-dev` repository, available at `https://github.com/blupi-games/planetblupi-dev.git`.
    * The `planetblupi-dev` repository serves as the main development bundle. It includes:
        - The `planetblupi` source code repository.
        - The `planetblupi-data` repository, which contains game assets.
        - Build scripts (CMake) and dependencies for creating distributable versions of the game.
...
4.  **Game Assets:** The `resources/` directory contains all the game's assets. This includes:
    *   `data/`: Game data, including mission files (`.blp` files) and configuration (`config.json`).
